...
<body class="bg-light">
	...
	<div class="container mt-5">
		...
		if (task == null) {
		%>
		<div class="alert alert-info mt-3">Task could not be found to
			edit.</div>
		<%
		} else {
		%>
		<form action="editTask" method="post">
			<input type="hidden" id="taskId" name="taskId"
				value="<%=task.getId()%>"> <input type="hidden" id="userId"
				name="userId" value="<%=session.getAttribute("userId")%>">
			...
			<div class="form-group">
				<label for="priority">Priority</label> <select class="form-control"
					...
					<option value="Low"
						<%=task.getPriority().equals("Low") ? "selected" : ""%>>Low</option>
					<option value="Medium"
						<%=task.getPriority().equals("Medium") ? "selected" : ""%>>Medium</option>
					<option value="High"
						<%=task.getPriority().equals("High") ? "selected" : ""%>>High</option>
				</select>
			</div>
